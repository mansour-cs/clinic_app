...
class _DoctorAppointmentsPageState extends State<DoctorAppointmentsPage> {
  ...
  Future<void> fetchAppointments() async {
    var response = await http.get(
      Uri.parse("http://10.206.6.18:8000/appointments/doctor/${widget.doctorId}"),
    );
    ...
    if (data["status"] == "success") {
      ...
      // ⭐ فلترة مواعيد اليوم إذا onlyToday = true
      if (widget.onlyToday) {
  ...
  void updateStatus(int appointmentId, String newStatus) async {
    var response = await http.post(
      Uri.parse("http://10.206.6.18:8000/appointments/update_status"),
...
class _AddNotesPageState extends State<AddNotesPage> {
  ...
  Future<void> saveNotes() async {
    var response = await http.post(
      Uri.parse("http://10.206.6.18:8000/appointments/add_notes"),
      headers: {"Content-Type": "application/x-www-form-urlencoded"},
      body: {
